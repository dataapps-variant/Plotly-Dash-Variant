"""
BigQuery Client - Plotly Dash Version
...
# App-level cache (persists across requests within same process)
_app_cache = {
    "data": None,
    "loaded_at": None,
    "date_bounds": None,
...
def load_date_bounds():
    """Load min and max dates from data"""
    global _app_cache
    
    if _app_cache.get("date_bounds"):
        return _app_cache["date_bounds"]
    ...
    _app_cache["date_bounds"] = result
    
    return result


def load_plan_groups(active_inactive="Active"):
    """Load unique App_Name + Plan_Name combinations"""
    global _app_cache
    
    cache_key = f"plan_groups_{active_inactive.lower()}"
    if _app_cache.get(cache_key):
    ...
    result = {"App_Name": unique_apps, "Plan_Name": unique_plans}
...
def load_all_chart_data(start_date, end_date, bc, cohort, plans, metrics, table_type, active_inactive="Active"):
    """
    Load ALL chart data in ONE pass instead of separate queries.
