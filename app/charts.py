"""
Chart Components for Variant Analytics Dashboard - Plotly Dash Version
...
def build_legend_html(plans, color_map, theme="dark"):
...
def build_line_chart(data, display_name, format_type="dollar", date_range=None, 
                     subscriptions_data=None, is_subscriptions_chart=False, theme="dark"):
    """
    ...
    Args:
        ...
        is_subscriptions_chart: If True, don't show Subscriptions in tooltip
        theme: 'dark' or 'light'
...
def create_chart_pair_layout(chart_data_regular, chart_data_crystal, display_name, 
                             format_type="dollar", date_range=None, chart_key="",
                             subscriptions_regular=None, subscriptions_crystal=None, theme="dark"):
    """
    Create layout for a pair of charts (Regular and Crystal Ball) side by side
    
    Returns a Dash html.Div component
    """
    ...
    legend_crystal = None
    if plans_regular:
        color_map_regular = build_plan_color_map(plans_regular)
        legend_regular = build_legend_html(plans_regular, color_map_regular, theme)
    if plans_crystal:
        color_map_crystal = build_plan_color_map(plans_crystal)
        legend_crystal = build_legend_html(plans_crystal, color_map_crystal, theme)
    ...
    return html.Div([
        html.Div([
            ...
            html.Div([
                ...
                legend_regular if legend_regular else html.Div(),
                dcc.Graph(
                    figure=fig_regular,
                    config=chart_config,
                    id=f'{chart_key}_regular'
                )
            ], style={'width': '50%', 'padding': '0 10px'}),
            
            # Crystal Ball Chart
            html.Div([
                html.Div(f"{display_name} (Crystal Ball)", style={
                    'fontSize': '16px',
                    'fontWeight': '600',
                    'color': colors['text_primary'],
                    'marginBottom': '12px'
                }),
                legend_crystal if legend_crystal else html.Div(),
                dcc.Graph(
                    figure=fig_crystal,
                    config=chart_config,
                    id=f'{chart_key}_crystal'
                )
            ], style={'width': '50%', 'padding': '0 10px'})
